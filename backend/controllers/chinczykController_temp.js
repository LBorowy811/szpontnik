...
class ChinczykController {
  ...
  rollDice(roomId, playerId) {
    ...
    const movablePawns = this.getMovablePawns(room.gameState, room.gameState.currentTurn, valu
e);

    if (movablePawns.length === 0 && value !== 6) {
      room.gameState.currentDice = null;
      room.gameState.currentTurn = (room.gameState.currentTurn + 1) % room.gameState.players.l
ength;
    }
  ...
  movePawn(roomId, playerId, pawnId) {
    ...
    const movablePawns = this.getMovablePawns(room.gameState, room.gameState.currentTurn, room
.gameState.currentDice);
  ...
  joinGameSocket({ gameId, username, userId, socketId }) {
    ...
    if (game.gameStarted) {
      console.log('[CHINCZYK] Gra rozpoczÄ™ta - uĹĽytkownik doĹ‚Ä…cza jako obserwator:', usern
ame);
      return { ok: true, game };
    }
    ...
    if (game.tournamentId && game.matchId) {
      ...
      const canJoin = tournamentController.canPlayerJoinMatch(game.tournamentId, game.matchId,
 userId);
    ...
    console.log('[CHINCZYK] Nowy gracz doĹ‚Ä…czyĹ‚:', username, 'Liczba graczy:', game.players
.length);
  ...
  rollDiceInternal(game, userId) {
    ...
    if (game.gameState.currentDice !== null) {
      console.log("[CHINCZYK BACKEND] BĹ‚Ä…d: Najpierw wykonaj ruch. currentDice =", game.game
State.currentDice);
      return { success: false, error: 'Najpierw wykonaj ruch' };
    }
    ...
    const movablePawns = this.getMovablePawns(game.gameState, game.gameState.currentTurn, valu
e);

    if (movablePawns.length === 0) {
      game.gameState.currentDice = null;
      game.gameState.currentTurn = (game.gameState.currentTurn + 1) % game.gameState.players.l
ength;
    }
  ...
  movePawnInternal(game, userId, pawnId) {
    ...
    const movablePawns = this.getMovablePawns(game.gameState, game.gameState.currentTurn, game
.gameState.currentDice);
    if (!movablePawns.find(p => p.id === pawnId)) {
      return { success: false, error: 'Nie moĹĽesz ruszyÄ‡ tego pionka' };
    }
    ...
    const moveEntry = {
      ...
      pawnId: pawnId,
      diceValue: diceValue,
      from: oldInHome ? 'DOM' : (oldGoalPosition >= 0 ? `META ${oldGoalPosition + 1}` : `pole 
${oldPathPosition + 1}`),
